...
class CVTest(TestCase):
    ...
    def setUp(self):
        self.admin_user = User.objects.create(username='jamesb')
    ...
    def test_cv_model_is_valid(self):
        model = CV.objects.create(author=self.admin_user)
        model.save()
    ...
    def test_qualification_model(self):
        cv = CV.objects.create(author=self.admin_user)
        cv.save()
    ...
    def test_employment_model(self):
        cv = CV.objects.create(author=self.admin_user)
        cv.save()
        employment = Employment.objects.create(linked_cv=cv)
        employment.save()
        self.assertEqual(employment.linked_cv,cv)
        self.assertEqual(employment.job_title,'')
        self.assertEqual(employment.start_date,date(1970,1,1))
        self.assertEqual(employment.end_date,date(1970,1,2))
        self.assertEqual(employment.description,'')
    
    def test_interest_model(self):
        cv = CV.objects.create(author=self.admin_user)
        cv.save()
        interest = Interest.objects.create(linked_cv=cv)
        interest.save()
        self.assertEqual(interest.linked_cv,cv)
